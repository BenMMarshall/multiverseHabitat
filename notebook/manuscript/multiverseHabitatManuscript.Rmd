...
Further details of how the scenarios were parametrised can be found in [@abmAnimalMovement], and also in the abmAnimalMovement github repository ([abmAnimalMovement Github](https://github.com/BenMMarshall/abmAnimalMovement/tree/main/notebook/manuscript)).
...
<!-- [LOAD IN AND LIST THE TRACKING DURATIONS HERE] -->
<!-- [LOAD IN AND LIST THE TRACKING FREQS HERE] -->
...
# knitr::include_graphics(here::here("notebook", "figures", "preregDecisionTrees.png"))
```
...
During the writing of this paper, a new method was described as part of the ctmm package <!-- add version and cite here-->, which we also preliminarily investigate.
...
There are suggestions that altering the weighing can improve model convergence and decrease the uncertainty surrounding habitat preference estimates [@fieberg_how_2021]; we included this decision to examine whether the pursuit of a more confident answer is biasing the point estimate.
...
The completion of the multiverse resulted in <!-- number of datasets --> `r length(list.files(here::here("_targets", "objects"), pattern = "sampDuraFreqData"))` datasets representing an individual animals movements sampled via a distinct tracking regime (i.e., a combination of tracking frequency and duration).
...
Each of these datasets underwent every combination of analysis choices resulting in <!-- number of estimates -->
`r scales::comma(dim(ssfResults)[1] + dim(areaResults)[1])` estimates of habitat preference (`r paste("Wides:", scales::comma(sum(areaResults$analysis == "wides")), ", RSF:", scales::comma(sum(areaResults$analysis == "rsf")), ", SSF:", scales::comma(dim(ssfResults)[1]))`).
...
<!-- spec curve wides -->
Starting with the Wides results, we see a quite gentle *S* shape [FIGURE LINK HERE], with most estimates falling in the range of 0.5 to 2.
...
<!-- spec curve rsf -->
The Resource Selection Function (RSF) specification curve shows a more ideal *S* shape, with the majority of estimates clearly converging on a common answer [FIGURE LINK HERE].
...
There is a decently clear *S* shape indicating many analysis end points resulting in similar estimates of preference.
...
library(dplyr)
brmWides_dEst_tdEffect <- areaBrms_wides$modOUT_dEst %>%
  tidybayes::spread_draws(b_tdScaled) %>%
  ggdist::median_hdci(.width = c(0.95))
brmWides_dEst_tfEffect <- areaBrms_wides$modOUT_dEst %>%
  tidybayes::spread_draws(b_tfScaled) %>%
  ggdist::median_hdci(.width = c(0.95))
brmWides_dEst_avpEffect <- areaBrms_wides$modOUT_dEst %>%
  tidybayes::spread_draws(b_availPointsPerScaled) %>%
  ggdist::median_hdci(.width = c(0.95))
brmWides_dEst_csEffect <- areaBrms_wides$modOUT_dEst %>%
  tidybayes::spread_draws(b_contourScaled) %>%
  ggdist::median_hdci(.width = c(0.95))

brmWides_rEst_tdEffect <- areaBrms_wides$modOUT_rEst %>%
  tidybayes::spread_draws(b_tdScaled) %>%
  ggdist::median_hdci(.width = c(0.95))
brmWides_rEst_tfEffect <- areaBrms_wides$modOUT_rEst %>%
  tidybayes::spread_draws(b_tfScaled) %>%
  ggdist::median_hdci(.width = c(0.95))
brmWides_rEst_avpEffect <- areaBrms_wides$modOUT_rEst %>%
  tidybayes::spread_draws(b_availPointsPerScaled) %>%
  ggdist::median_hdci(.width = c(0.95))
brmWides_rEst_spEffect <- areaBrms_wides$modOUT_rEst %>%
  tidybayes::spread_draws(b_samplingPattern) %>%
  ggdist::median_hdci(.width = c(0.95))
```

<!-- model res wides absolute -->
For the decisions connected to Wides results, the absolute deviation was curtailed most effectively by increases in data quantity, with increases in tracking duration ($\beta$ `r paste0(round(brmWides_dEst_tdEffect[1], digits = 3), ", 95% median Highest-Density Continuous Interval CrI ", round(brmWides_dEst_tdEffect[2], digits = 3), " – ", round(brmWides_dEst_tdEffect[3], digits = 3))`) being marginally more effective than tracking frequency ($\beta$ `r paste0(round(brmWides_dEst_tfEffect[1], digits = 3), ", 95% CrI ", round(brmWides_dEst_tfEffect[2], digits = 3), " – ", round(brmWides_dEst_tfEffect[3], digits = 3))`).
Of the other decisions only the multiplier of available points appeared to decrease absolute deviation unambiguously ($\beta$ `r paste0(round(brmWides_dEst_avpEffect[1], digits = 3), ", 95% CrI ", round(brmWides_dEst_avpEffect[2], digits = 3), " – ", round(brmWides_dEst_avpEffect[3], digits = 3))`), and it's effect was much smaller than those concerning data quantity.
...
Contour choice increases the absolute deviation from the median estimate $\beta$ `r paste0(round(brmWides_dEst_csEffect[1], digits = 3), ", 95% CrI ", round(brmWides_dEst_csEffect[2], digits = 3), " – ", round(brmWides_dEst_csEffect[3], digits = 3))`, that appears counter to the initial specification curve plot and highlights the difficulty of identifying patterns from the raw data alone. 

<!-- model res wides raw -->
The model examining raw deviation from the median estimate reveals that the Wides approach is contrastingly impacted by tracking frequency $\beta$ `r paste0(round(brmWides_rEst_tfEffect[1], digits = 3), ", 95% CrI ", round(brmWides_rEst_tfEffect[2], digits = 3), " – ", round(brmWides_rEst_tfEffect[3], digits = 3))` and tracking duration $\beta$ `r paste0(round(brmWides_rEst_tdEffect[1], digits = 3), ", 95% CrI ", round(brmWides_rEst_tdEffect[2], digits = 3), " – ", round(brmWides_rEst_tdEffect[3], digits = 3))`, with the former leading to greater estimates of preference.
...
The impact of stratified versus random available point positioning $\beta$ `r paste0(round(brmWides_rEst_spEffect[1], digits = 3), ", 95% CrI ", round(brmWides_rEst_spEffect[2], digits = 3), " – ", round(brmWides_rEst_spEffect[3], digits = 3))` and the available points multiplier $\beta$ `r paste0(round(brmWides_rEst_avpEffect[1], digits = 3), ", 95% CrI ", round(brmWides_rEst_avpEffect[2], digits = 3), " – ", round(brmWides_rEst_avpEffect[3], digits = 3))` have little impact mirror the lack of impact in the absolute deviation model. 
...
library(dplyr)
brmrsf_dEst_tdEffect <- areaBrms_rsf$modOUT_dEst %>%
  tidybayes::spread_draws(b_tdScaled) %>%
  ggdist::median_hdci(.width = c(0.95))
brmrsf_dEst_tfEffect <- areaBrms_rsf$modOUT_dEst %>%
  tidybayes::spread_draws(b_tfScaled) %>%
  ggdist::median_hdci(.width = c(0.95))
brmrsf_dEst_csEffect <- areaBrms_rsf$modOUT_dEst %>%
  tidybayes::spread_draws(b_contourScaled) %>%
  ggdist::median_hdci(.width = c(0.95))
brmrsf_dEst_avpEffect <- areaBrms_rsf$modOUT_dEst %>%
  tidybayes::spread_draws(b_availPointsPerScaled) %>%
  ggdist::median_hdci(.width = c(0.95))
brmrsf_dEst_spEffect <- areaBrms_rsf$modOUT_dEst %>%
  tidybayes::spread_draws(b_samplingPatternst) %>%
  ggdist::median_hdci(.width = c(0.95))
brmrsf_dEst_akdeEffect <- areaBrms_rsf$modOUT_dEst %>%
  tidybayes::spread_draws(b_areaAKDE) %>%
  ggdist::median_hdci(.width = c(0.95))
brmrsf_dEst_kdeEffect <- areaBrms_rsf$modOUT_dEst %>%
  tidybayes::spread_draws(b_areaKDEhref) %>%
  ggdist::median_hdci(.width = c(0.95))
brmrsf_dEst_dbmEffect <- areaBrms_rsf$modOUT_dEst %>%
  tidybayes::spread_draws(b_areadBBMM) %>%
  ggdist::median_hdci(.width = c(0.95))

brmrsf_rEst_tdEffect <- areaBrms_rsf$modOUT_rEst %>%
  tidybayes::spread_draws(b_tdScaled) %>%
  ggdist::median_hdci(.width = c(0.95))
brmrsf_rEst_tfEffect <- areaBrms_rsf$modOUT_rEst %>%
  tidybayes::spread_draws(b_tfScaled) %>%
  ggdist::median_hdci(.width = c(0.95))
brmrsf_rEst_spEffect <- areaBrms_rsf$modOUT_rEst %>%
  tidybayes::spread_draws(b_samplingPatternst) %>%
  ggdist::median_hdci(.width = c(0.95))
brmrsf_rEst_avpEffect <- areaBrms_rsf$modOUT_rEst %>%
  tidybayes::spread_draws(b_availPointsPerScaled) %>%
  ggdist::median_hdci(.width = c(0.95))
brmrsf_rEst_dbmEffect <- areaBrms_rsf$modOUT_rEst %>%
  tidybayes::spread_draws(b_areadBBMM) %>%
  ggdist::median_hdci(.width = c(0.95))
brmrsf_rEst_csEffect <- areaBrms_rsf$modOUT_rEst %>%
  tidybayes::spread_draws(b_contourScaled) %>%
  ggdist::median_hdci(.width = c(0.95))
```
...
The two largest effects are from tracking duration ($\beta$ `r paste0(round(brmrsf_dEst_tdEffect[1], digits = 3), ", 95% CrI ", round(brmrsf_dEst_tdEffect[2], digits = 3), " – ", round(brmrsf_dEst_tdEffect[3], digits = 3))`) and frequency ($\beta$ `r paste0(round(brmrsf_dEst_tfEffect[1], digits = 3), ", 95% CrI ", round(brmrsf_dEst_tfEffect[2], digits = 3), " – ", round(brmrsf_dEst_tfEffect[3], digits = 3))`), showing that more data leads to more stable estimates of habitat preference (i.e., more likely to get an answer closer to the overall median estimate).

However, unlike with Wides the other analysis decisions also matter, although to varying extents.
It appears that larger contours ($\beta$ `r paste0(round(brmrsf_dEst_csEffect[1], digits = 3), ", 95% CrI ", round(brmrsf_dEst_csEffect[2], digits = 3), " – ", round(brmrsf_dEst_csEffect[3], digits = 3))`) and a greater multiplier of available points ($\beta$ `r paste0(round(brmrsf_dEst_avpEffect[1], digits = 3), ", 95% CrI ", round(brmrsf_dEst_avpEffect[2], digits = 3), " – ", round(brmrsf_dEst_avpEffect[3], digits = 3))`) decreases the deviation from the median estimate.
To a small extent using a stratified available points pattern lead to lower deviation from the median than a purely random available points pattern ($\beta$ `r paste0(round(brmrsf_dEst_spEffect[1], digits = 3), ", 95% CrI ", round(brmrsf_dEst_spEffect[2], digits = 3), " – ", round(brmrsf_dEst_spEffect[3], digits = 3))`).
Choice of area method can alter results in two ways.
Compared to using MCPs both AKDE ($\beta$ `r paste0(round(brmrsf_dEst_akdeEffect[1], digits = 3), ", 95% CrI ", round(brmrsf_dEst_akdeEffect[2], digits = 3), " – ", round(brmrsf_dEst_akdeEffect[3], digits = 3))`) and KDEhref ($\beta$ `r paste0(round(brmrsf_dEst_kdeEffect[1], digits = 3), ", 95% CrI ", round(brmrsf_dEst_kdeEffect[2], digits = 3), " – ", round(brmrsf_dEst_kdeEffect[3], digits = 3))`) produce estimates more likely to be closer to the median estimate, and to similar degrees.
Dynamic Brownian Bridge Movement Models on the other hand are more likely to lead to greater variation in estimates than those analyses that use MCPs ($\beta$ `r paste0(round(brmrsf_dEst_dbmEffect[1], digits = 3), ", 95% CrI ", round(brmrsf_dEst_dbmEffect[2], digits = 3), " – ", round(brmrsf_dEst_dbmEffect[3], digits = 3))`).
...
<!-- need to add some numbers here for interactions, likely gradients to describe sensitivity, also finalise figure -->


<!-- model res rsf raw -->
The RSF model examining raw deviation from the median estimate reveals a general tendency for higher data quantity to leading to more conservative estimates of preference (tracking duration $\beta$ `r paste0(round(brmrsf_rEst_tdEffect[1], digits = 3), ", 95% CrI ", round(brmrsf_rEst_tdEffect[2], digits = 3), " – ", round(brmrsf_rEst_tdEffect[3], digits = 3))` and tracking frequency $\beta$ `r paste0(round(brmrsf_rEst_tfEffect[1], digits = 3), ", 95% CrI ", round(brmrsf_rEst_tfEffect[2], digits = 3), " – ", round(brmrsf_rEst_tfEffect[3], digits = 3))`). 
...
Stratified available point sampling was associated with more positive estimates of preference ($\beta$ `r paste0(round(brmrsf_rEst_spEffect[1], digits = 3), ", 95% CrI ", round(brmrsf_rEst_spEffect[2], digits = 3), " – ", round(brmrsf_rEst_spEffect[3], digits = 3))`), but also those closer to the median (as indicated by the previous model).
This suggests that random point sampling, by contrast, leads to more variable estimates that tend to be lower.
The same pattern is visible in available points multiplier, where more points lead to a more consistent estimate but with a tendency for that estimate to be higher ($\beta$ `r paste0(round(brmrsf_rEst_avpEffect[1], digits = 3), ", 95% CrI ", round(brmrsf_rEst_avpEffect[2], digits = 3), " – ", round(brmrsf_rEst_avpEffect[3], digits = 3))`).

The area method choices reveal a similar pattern.
Dynamic Brownian Bridge Movement models were connected to the more variable estimates, but this model reveals that the variability did not tend towards a particular direction ($\beta$ `r paste0(round(brmrsf_rEst_dbmEffect[1], digits = 3), ", 95% CrI ", round(brmrsf_rEst_dbmEffect[2], digits = 3), " – ", round(brmrsf_rEst_dbmEffect[3], digits = 3))`).
Whereas the other methods are connected to lower deviation from the median estimate, but higher raw estimates.
The area contour selected also reflects this ($\beta$ `r paste0(round(brmrsf_rEst_csEffect[1], digits = 3), ", 95% CrI ", round(brmrsf_rEst_csEffect[2], digits = 3), " – ", round(brmrsf_rEst_csEffect[3], digits = 3))`).
...
library(dplyr)
brmssf_dEst_tdEffect <- ssfBrms$modOUT_dEst %>%
  tidybayes::spread_draws(b_tdScaled) %>%
  ggdist::median_hdci(.width = c(0.95))
brmssf_dEst_tfEffect <- ssfBrms$modOUT_dEst %>%
  tidybayes::spread_draws(b_tfScaled) %>%
  ggdist::median_hdci(.width = c(0.95))

brmssf_rEst_sdEffect <- ssfBrms$modOUT_rEst %>%
  tidybayes::spread_draws(b_stepDistgamma) %>%
  ggdist::median_hdci(.width = c(0.95))
```

<!-- model res ssf absolute -->

The SSF absolute deviation model matches very closely the previous two models, with tracking quantity both in terms of duration ($\beta$ `r paste0(round(brmssf_dEst_tdEffect[1], digits = 3), ", 95% CrI ", round(brmssf_dEst_tdEffect[2], digits = 3), " – ", round(brmssf_dEst_tdEffect[3], digits = 3))`) and frequency ($\beta$ `r paste0(round(brmssf_dEst_tfEffect[1], digits = 3), ", 95% CrI ", round(brmssf_dEst_tfEffect[2], digits = 3), " – ", round(brmssf_dEst_tfEffect[3], digits = 3))`) dominate in regards to the explaining why estimates tend to be closer to the median estimate.
...
The only noticeable difference is in the step distribution decision that changes the direction of the beta point estimate from negative to positive ($\beta$ `r paste0(round(brmssf_rEst_sdEffect[1], digits = 3), ", 95% CrI ", round(brmssf_rEst_sdEffect[2], digits = 3), " – ", round(brmssf_rEst_sdEffect[3], digits = 3))`).
...
library(dplyr)
brmwrsf_dEst_tdEffect <- wrsfBrms$modOUT_dEst %>%
  tidybayes::spread_draws(b_tdScaled) %>%
  ggdist::median_hdci(.width = c(0.95))
brmwrsf_dEst_tfEffect <- wrsfBrms$modOUT_dEst %>%
  tidybayes::spread_draws(b_tfScaled) %>%
  ggdist::median_hdci(.width = c(0.95))

brmwrsf_rEst_tdEffect <- wrsfBrms$modOUT_eEst %>%
  tidybayes::spread_draws(b_tdScaled) %>%
  ggdist::median_hdci(.width = c(0.95))
brmwrsf_rEst_tfEffect <- wrsfBrms$modOUT_rEst %>%
  tidybayes::spread_draws(b_tfScaled) %>%
  ggdist::median_hdci(.width = c(0.95))
```

The results from the WRSF models are largely similar to the SSF models, although with fewer decisions.
Both tracking duration and tracking frequency lead to lower deviation from the median estimate (tracking duration $\beta$ `r paste0(round(brmwrsf_dEst_tdEffect[1], digits = 3), ", 95% CrI ", round(brmwrsf_dEst_tdEffect[2], digits = 3), " – ", round(brmwrsf_dEst_tdEffect[3], digits = 3))` and tracking frequency $\beta$ `r paste0(round(brmwrsf_dEst_tfEffect[1], digits = 3), ", 95% CrI ", round(brmwrsf_dEst_tfEffect[2], digits = 3), " – ", round(brmwrsf_dEst_tfEffect[3], digits = 3))`), and the nearly exclusively positively skewed outliers mean that lower raw deviation is associated with lower estimates (tracking duration $\beta$ `r paste0(round(brmwrsf_rEst_tdEffect[1], digits = 3), ", 95% CrI ", round(brmwrsf_rEst_tdEffect[2], digits = 3), " – ", round(brmwrsf_rEst_tdEffect[3], digits = 3))` and tracking frequency $\beta$ `r paste0(round(brmwrsf_rEst_tfEffect[1], digits = 3), ", 95% CrI ", round(brmwrsf_rEst_tfEffect[2], digits = 3), " – ", round(brmwrsf_rEst_tfEffect[3], digits = 3))`).
...
The overall pattern in estimates mirror those seen in other fields, where the bulk of answers converge but outliers remain. 
<!-- [EXAMPLES HERE OF OTHER FIELDS] -->
While the incentives for findings exciting results remain in the publishing system <!-- [CITES] -->, using analysis choices to game or mine for a given exciting result appears time consuming because the proportion of the extreme estimates is relatively low compared to the majority of estimates that agree near the median.
Therefore, we can be as confident in past studies findings as we would those from <!-- [EXAMPLE FIELDS HERE] -->.
...
In particular the benefits of larger datasets and the importance of defining availability [@street_solving_2021; @northrup_practical_2013], latter particularly key because of the potential for subjective definitions [@beyer_interpretation_2010]. 
One specific example would be the tendency for larger area methods, and wider contours leading to more stable (nearer the median) estimates; this mirrors previously reveals patterns connecting broader spatial scales and transferability [@paton_defining_2015].
...
Increases in the number of available points will improve the accuracy when defining available habitats; [@street_solving_2021], indicates that the systematic sampling with high numbers of points is preferable to capture a complete unbiased availability.
...
<!-- sclae is important always because of interactions between sampling and question/behaviour. Thurfjell -->
...
We cannot discount scenarios with different movement characteristics that interact with particular analysis choices to bias habitat selection estimates –scale remains a key consideration for any study [@thurfjell_applications_2014; @northrup_conceptual_2022], and context-specific selection remains difficult to translate [@avgar_habitat_2020].
