...
Researchers are inextricably part of the research process [@levins_dialectical_1985; @tang-martinez_history_2020], and our expectations can shape our conclusions [@holman_evidence_2015].
...
We draw on our own experience, and the input of peers, to try and ensure the best decisions are made to produce robust and reliable results, but we also contend with data, expertise, and interpretability constraints [@liu_paths_2020]. 
...
Similarly, novelty has been promoted by journals to the detriment of replication [@forstmeier_detecting_2017; @vinkers_use_2015; @brembs_reliable_2019], despite widespread agreement on the importance of replications [@fraser_role_2020].
...
Questionable research practices can be broadly viewed as methods to achieve a neat, statistically significant, and publishable narrative [@oboyle_chrysalis_2014].
...
Even in the absence of any undesirable incentives, analysis flexibility can still lead to variable results simply as the result of researchers considering approaches of differing validities for a given dataset [@gelman_garden_2013; @gould_same_2023].
...
The most general category is conceptual replications, where the subject and method of study varies from the original study, but the replication targets the same concept or hypothesis [@kelly_rate_2019; @nakagawa_replicating_2015].
...
This reveals how the choices during analysis (e.g., choice of area estimation method) impact results, and how the uncertainty introduced by those choices change depending on the sampling.
It presents a scenario where the _correct_ choice is dependent on preceding decisions; thereby, highlighting the need to explore the impacts of multiple decisions simultaneously. 

As seen in the reptile space use example, the choices available to the researcher [i.e., researcher degrees of freedom, @simmons_false-positive_2011] is a key source of variation among studies.
...
Naturally each participant takes a slightly different approach, both in how the question is interpreted [@auspurg_has_2021], and the analysis approach chosen [@bastiaansen_time_2020; @gelman_garden_2013; @gould_same_2023], resulting in different final results.
...
An approach to address the unknown impacts of undisclosed researcher degrees of freedom is to fully explore all plausible or reasonable analysis choices open to researchers, i.e., to explore a multiverse of design choices [@steegen_increasing_2016].
This multiverse analysis --closely linked to vibration of effects [@patel_assessment_2015], multi-model analysis [@young_model_2017], and specification curve analysis [@simonsohn_specification_2020]-- has the potential to quantify the variation stemming from researchers' analyses choices [@rijnhart_assessing_2021].
...
Construction of a multiverse requires justification for which decisions are treated as variable, and why there is not an _a priori_ and defensible single solution [@del_giudice_travelers_2021].
...
However, even if all choices are equally valid, multiverse analysis cannot simply provide a correct answer [@steegen_increasing_2016].
...
As mentioned, ecological systems are complex to study and frustrate replication efforts [@nakagawa_replicating_2015; @schnitzer_would_2016]; this can be especially true in the case of movement ecology and animal movement.
...
Here we construct a multiverse of analyses to explore how decisions concerning the design and analysis of animal tracking data can impact the findings on habitat selection.
...
Further details of how the scenarios were parametrised can be found in @abmAnimalMovement, and also in the abmAnimalMovement github repository along with associated documentation: [https://github.com/BenMMarshall/abmAnimalMovement](https://github.com/BenMMarshall/abmAnimalMovement/tree/main/notebook/manuscript), also archived: [https://doi.org/10.5281/zenodo.6951937](https://doi.org/10.5281/zenodo.6951937).
...
* Species 3, King Cobra: lower site fidelity making use of many shelter sites, a medium movement speed through a landscape with high resistance barriers, an 8-12 hour activity cycle combined with an approximately weekly forage-digest cycle and a weak seasonal cycle.
...
The full parametrisation of the simulated species can be found in the code associated with this paper, specifically the simulate_individual.R function (available at: [https://github.com/BenMMarshall/multiverseHabitat](https://github.com/BenMMarshall/multiverseHabitat), and archived at: [https://doi.org/10.5281/zenodo.12169335](https://doi.org/10.5281/zenodo.12169335)).
...
From the initial independent Gaussian fields, we altered the values to exaggerate the difference between high resource areas and low value areas; thereby generating non-independence of resources. 
Broadly, we created: core resource areas with higher foraging quality but lower movement ease, edge areas that overlap with the core resource area with better movement ease, and more barren areas with high movement ease but with minimal foraging value.
...
```{r landscapeExample, echo=FALSE, out.width='100%', fig.height=5, fig.width=3, fig.align="centre", fig.cap="An example of the underlying environmental matricies used during the Badger species simulation, and the subsequent habitat classes to be used in the habitat selection analysis."}
...
For this exploration, we assumed that the researcher has sufficient knowledge about the animals' ecology to prioritise tracking during active hours.
...
Therefore, for the lower tracking frequencies we ensured that daily tracking was centred the animals' active period (as all our simulated species had daily activity patterns).
...
Where the combination of sampling frequency and duration resulted in fewer than 30 data points, we did not conduct analysis as this would cause issues for available area creation.
...
@joo_navigating_2020 provide a review of the packages available to analyse movement data in R, and we used this review as a resource for determining the options for habitat analysis.
...
* From the ctmm package: the newly described Weighted Resource Selection Function [wRSF @alston_mitigating_2023].
...
The Wides approach aims to calculate selection ratios of used habitat compared to those available [@manly2007resource]; whereas the RSF approach uses a logistic regression to compare the used and available habitats.
Therefore, a principal concern in both approaches is how available habitat is defined.

The first decision is whether to approach the habitat selection analysis as a type I (population habitat use compared to landscape-level habitat availability), II (individual habitat use compared to landscape-level habitat availability) or III design (individual habitat use compared to individual-level habitat availability).
We are concerned with an individual's habitat selection, so we ignored type I.
...
Type III and the need to individually define available habitat opens up many different analytical decisions concerning how that availability is defined.
...
Minimum convex polygons (MCPs) simply create a polygon based on the outermost locations, where the interior angles sum to less than 180 degrees.
...
For this analysis, we use the reference bandwidth (href) as it is commonly used [@crane_lots_2021], while also quickly and consistently calculable on an individual-by-individual basis.
...
MCPs areas are generated based on a percentage of the location points, whereas the KDE, AKDE, and dBBMM methods require a contour to be extracted from the utilisation distribution (occurrence distribution for dBBMMs).
...
The first model provides us with an idea of the decisions that can lead to random, but potentially unbiased variation surrounding the median estimate; whereas the second model highlights decisions that lead to systematic over or under estimation of habitat selection.
...
```{r specCurveWides, echo=FALSE, out.width='100%', fig.height=5, fig.width=3, fig.align="centre", fig.cap="A specification curve showing all habitat selection estimates from the Wides analysis pathways applied to the simulated data. Each point is an estimate of habitat selection. Top plot shows all estimates according to estimate, with an arbitary sorted y-position. Lower plot shows the same estimates in relation to the decisions that lead to them. Left-hand numbers describe the number of NA results connected to each decision, indicating a failure somewhere in the calculations. Solid circles are the medians for each choice."}
...
```{r specCurveRSF, echo=FALSE, out.width='100%', fig.height=5, fig.width=3, fig.align="centre", fig.cap="A specification curve showing all habitat selection estimates from the RSF analysis pathways applied to the simulated data. Each point is a estimate of habitat selection. Top plot shows all estimates according to estimate, with an arbitary sorted y-position. Lower plot show the same estimates in relation to the decisions that lead to them. Solid circles are the medians for each choice. Colours illustrate deviation from the median."}
knitr::include_graphics(here::here("notebook", "figures", "rsfSpecCurve.png"))
```

<!-- spec curve rsf -->
The Resource Selection Function (RSF) specification curves show a more ideal *S* shape, with the majority of estimates clearly converging on a common answer (Fig. \@ref(fig:specCurveRSF)).
...
Although weak patterns, tracking frequency, and multiplier of available points see slight reductions in the number of estimates that are very high or very low.
...
```{r uncertaintyPlot, echo=FALSE, out.width='100%', fig.height=3, fig.width=2, fig.align="centre", fig.cap="All standard errors associated with estimates of habitat selection from RSF, SSF, and wRSF analysis pathways. More extreme estimates of selection are associated with dramatically larger confidence intervals, N.b., y axis is log scaled."}
...
```{r specCurveSSF, echo=FALSE, out.width='100%', fig.height=5, fig.width=3, fig.align="centre", fig.cap="A specification curve showing all habitat selection estimates from the SSF analysis pathways applied to the simulated data. Each point is an estimate of habitat selection. Top plot shows all estimates according to estimate, with an arbitary sorted y-position. Lower plot shows the same estimates in relation to the decisions that lead to them. Solid circles are the medians for each choice. Colours illustrate deviation from the median."}
knitr::include_graphics(here::here("notebook", "figures", "ssfSpecCurve.png"))
```

<!-- spec curve ssf -->

The Step Selection Function (SSF) specification curves appear even more successful than the RSF.
...
```{r specCurveWRSF, echo=FALSE, out.width='100%', fig.height=5, fig.width=3, fig.align="centre", fig.cap="A specification curve showing all habitat selection estimates from the wRSF analysis pathways applied to the simulated data. Each point is an estimate of habitat selection. Top plot shows all estimates according to estimate, with an arbitary sorted y-position. Lower plot show the same estimates in relation to the decisions that lead to them. Solid circles are the medians for each choice. Colours illustrate deviation from the median."}
...
As with RSF and SSF methods, the wRSF is very likely to correctly identify the positive selection.
Also the median selection under the no selection scenario is correctly estimated as lower, particularly when more data are available.
...
Every decision examined by the RSF absolute effect model appears to make a difference when estimating selection (Fig. \@ref(fig:brmsAllEffectsPlot); Fig. \@ref(fig:dEstBetasRSF)).
...
Dynamic Brownian Bridge Movement models were connected to the more variable estimates, and that variability tended towards lower estimates ($\beta$: `r brmrsf_rEst_dbmEffect`).
...
Unlike the other analyses, all other decisions made a negligible impact on an estimate's absolute proximity to the median estimate.
...
However, the large overlap with zero means that in both cases the step distribution decision is not making a considerable impact on the estimates relative to the median estimate.
...
```{r percentile, echo=FALSE, message=FALSE, warning=FALSE}
areaQuant <- areaResults %>% 
  group_by(classLandscape, analysis) %>% 
  summarise(q10 = quantile(Estimate, probs = 0.1, na.rm = TRUE),
            q90 = quantile(Estimate, probs = 0.9, na.rm = TRUE)) %>% 
  mutate(inText = paste0(ifelse(analysis == "rsf", "RSF ", "Wides "),
                         ifelse(classLandscape == "classRaster", "selection", "scrambled"),
                         ": ",
                         "10% ", signif(q10, digits = 3), ", ",
                         "90% ", signif(q90, digits = 3))) %>% 
...
ssfQuant <- ssfResults %>% 
  ...
  summarise(q10 = quantile(Estimate, probs = 0.1, na.rm = TRUE),
            q90 = quantile(Estimate, probs = 0.9, na.rm = TRUE)) %>% 
  mutate(inText = paste0("SSF ", ifelse(classLandscape == "classRaster", "selection", "scrambled"),
                         ": ",
                         "10% ", signif(q10, digits = 3), ", ",
                         "90% ", signif(q90, digits = 3)))

wrsfQuant <- wrsfResults %>% 
  group_by(classLandscape) %>% 
  summarise(q10 = quantile(Estimate, probs = 0.1, na.rm = TRUE),
            q90 = quantile(Estimate, probs = 0.9, na.rm = TRUE)) %>% 
  mutate(inText = paste0("wRSF ", ifelse(classLandscape == "classRaster", "selection", "scrambled"),
                         ": ",
                         "10% ", signif(q10, digits = 3), ", ",
                         "90% ", signif(q90, digits = 3)))
  
fullInTextQuant <- paste(c(areaQuant$inText, ssfQuant$inText, wrsfQuant$inText), collapse = "; ")
```
...
The overall pattern in estimates mirrors those seen in other fields, where the bulk of answers converge but outliers remain [@silberzahn_many_2018; @simonsohn_specification_2020; @gould_same_2023].
While the incentives for finding positive results remain in the publishing system [@fanelli_positive_2010; @fanelli_negative_2012], in the case presented here, using analysis choices to game or mine for a given exciting result appears time-consuming because the proportion of extreme estimates is relatively low compared to the majority of estimates that agree near the median.
For example, the 10 and 90% percentiles easily exclude outlying estimates, particularly those lower estimates, and remain reasonably close to the median estimates `r fullInTextQuant`.
...
Our simulations contained daily cycles and the tracking spacing was skewed towards those active hours in those cycles for lower tracking frequency regimes.
...
Increases in the number of available points will improve accuracy when defining available habitats; @street_solving_2021 indicate that the systematic sampling with high numbers of points is preferable to capture a complete unbiased availability.
...
Despite the inherent limitations of multiverses, the exploration of analysis choice presents an important consideration for any researcher looking to know the reliability of their findings [@hoffmann_multiplicity_2021].
...
A focus on these approaches will allows us to explore the third key component of data quantity –sample size– as well as how well analyses tackle individual variability, either in the analysis itself or while summarising individual habitat selection estimates.
There are already methods for assessing required sample size to detect a given selection for RSFs [@street_solving_2021].
...
The code used to simulate the movement data, perform the multiverse, and analyse the results is available at: [https://github.com/BenMMarshall/multiverseHabitat](https://github.com/BenMMarshall/multiverseHabitat), and archived at [https://doi.org/10.5281/zenodo.12169335](https://doi.org/10.5281/zenodo.12169335).
...
write.csv(tableData, here::here("notebook", "tables", "Table S1 - r2 results.csv"), row.names = FALSE)
...
directEstimates <- read.csv(here::here("notebook", "modelOutput", "directEstimates.csv"))

tableDataDirect <- directEstimates %>%
    dplyr::mutate(across(where(is.numeric), round, digits = 3)) %>%
    dplyr::mutate(scale = str_to_sentence(scale)
    ) %>%
    dplyr::select(-X)

write.csv(tableDataDirect, here::here("notebook", "tables", "Table S2 - direct estimates.csv"), row.names = FALSE)

knitr::kable(
  tableDataDirect,
  caption = "Analysis results when applied directly to un-resampled movement data at both the decision timescale and movement timescales.",
  format = "latex",
  align = "lcc",
  vline = "",
  toprule = "",
  ...
  linesep = "",
  bottomrule = "",
  col.names = c("Estimate", "Standard error", "Analysis method", "Simulation scale", "Individual",
                "Species"), escape = FALSE) %>%
  kableExtra::kable_styling(font_size = 6, latex_options = "scale_down")
```
