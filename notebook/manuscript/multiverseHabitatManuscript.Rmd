...
A decrease in robustness can be seen in the increases in effect sizes inflation, p-value misreporting, and other measures of quality [@Brembs2018].
...
Questionable research practices can be viewed as methods to achieve a neat, statistically significant, and publishable narrative [@oboyle_chrysalis_2014].
...
This flexibility helps enable questionable research practices [@fraser_questionable_2018], and is potentially steered by publication bias [@cassey_survey_2004; @jennions_publication_2002] if more publishable results are prioritised/rewarded over less exciting but robust results.
...
Analysis flexibility can still lead to variable results in the absence of any undesirable incentives simply as the result of researchers considering different approaches of differing validities for a given dataset [@gelman_garden_2013; @gould_same_2023].
...
Since ecological systems are also complex and in constant flux, often frustrating perfect replications due to changes in space and time [@nakagawa_replicating_2015; @schnitzer_would_2016], researchers are left with a distinct lack of direct replications in ecology [@kelly_rate_2019].
...
As seen in the reptile space use example, the choices made by the researcher [ie., researcher degrees of freedom, @simmons_false-positive_2011] is a key source of variation among studies.
...
A rising approach addressing the unknown impacts of undisclosed researcher degrees of freedom is to fully explore all plausible or reasonable analysis choices open to researchers -- to explore a multiverse of design choices [@steegen_increasing_2016].
...
```{r multiDiagram, echo=FALSE, out.width='100%', fig.height=5, fig.width=3, fig.align="centre", fig.cap="Diagram showing how multiverse analysis differs from other approaches. Each branch node represents a choice made during analysis. Redrawn from Dragicevic et al. (2019)."}
...
Further details of how the scenarios were parametrised can be found in [@abmAnimalMovement], and also in the abmAnimalMovement github repository along with associated documentation ([abmAnimalMovement Github](https://github.com/BenMMarshall/abmAnimalMovement/tree/main/notebook/manuscript)).
...
<!-- POSSIBLE EXPANSION - The current multiverse(s) are examining movement data with a known positive preference, but it would be very easy to re-run the whole thing with no preference. I think this could be done by just scrambling the classified raster used in the habitat preference analyses –breaking any connection between the movement (and the underlying foraging, resting, movement layers) and the simplified layers used for the analyses. I am going to focus on getting what I have tidied up and the preprint better before exploring this, but it does strike me as an interesting and simple angle to explore: what does the variation look like when there is no preference? Adding this to the paper I think would give more of an insight into whether the variation can cause false positives; as it stands now, the papers are more about spotting the variation causing false negatives (given a certain hard to define level of habitat preference). So in lieu of being able to run a suite of scenarios with different habitat preference strengths, a comparison of zero preference to some-positive preference might be the next best thing. -->

<!-- make the text and overall feel of the figure larger and clearer, question the need for the x and y axes -->
...
We varied tracking duration from `r paste0(min(optionsCompleteList$regime$td), " to ", max(optionsCompleteList$regime$td))` days, and tracking frequency from `r paste0(min(optionsCompleteList$regime$tf), " to ", max(optionsCompleteList$regime$tf))` data points every hour (i.e., `r paste0(round(1/min(optionsCompleteList$regime$tf), digits = 2), " to ", round(1/ max(optionsCompleteList$regime$tf), digits = 2))` points per hour) .
...
In summary, the sampling decisions will cover variation in species represented by four independent individuals, tracking frequency, and tracking duration (Fig. \@ref(fig:decisionTrees)).

```{r decisionTrees, echo=FALSE, out.width='100%', fig.height=5, fig.width=3, fig.align="centre", fig.cap="An illustration of the different decisions creating the multiverse, split between those pertaining to sampling and analysis. Grey choices pertaining to data simulation and sampling. Species: the three different movement archetypes the simulations were loosely based upon. Individual: the number of individuals simulated per each species. Tracking duration: the length in days of the tracking data sampled from the overall simulated year. Tracking frequency: the frequency of the sampling of data points from the 1-per-minute simulated data. Purple choices concerned with analysis pathways using area-based estimations of availability (Wides: Wides selection ratios, RSF: Resource Selection Function). Area method: the method used to generate the area polygon that is considered available to the individual (MCP: Minimum Convex Polygon, KDE: Kernel Density Estimator, AKDE: Autocorrelated Kernel Density Estimator, dBBMM: dynamic Brownian Bridge Movement Models). Contour: the threshold used to convert the utilisation/occurrence distribution into a discrete polygon. Available points: the number of randomly generated points to extract available habitat within the polygon. Points pattern: whether the random points were spatially distributed in a stratified or pure random fashion. Weighting: the weighting of the available points in relation to the used points in the RSF formula. Light orange choices pertain to the SSF (Step Selection Function) analysis pathways. Model formula: whether the step and turn angles were added as interactions to the habitat variable (integrated if added). Available points: the number of randomly generated available points per step/used point. Turn angle: the distribution used to generate the turn angles of the available points. Step distribution: the distribution used to generate the step distances of the available points. Dark orange is the WRSF (Weighted Resource Selection Function) analysis. Decisions that required discrete choices are illustrated with circles, whereas those that continuous are depicted as triangles."}
...
During the writing of this paper, a new method was described as part of the ctmm package `r paste0("v.", packageVersion("ctmm"))` [@ctmm], which we also preliminarily investigate.
...
Therefore, we use the guidance from [@silva_autocorrelationinformed_2022] to generate weighted AKDEs using perturbative hybrid residual maximum likelihood method (pHREML), and select the best performing by AICc for inclusion in further analysis.
Using weighted AKDEs means the area estimates better address gaps between locations [@silva_autocorrelationinformed_2022], with no apparent drawbacks nor large computational cost. 
...
As our most infrequent tracking is `r max(optionsCompleteList$regime$tf)` hours (`r max(optionsCompleteList$regime$tf)/24` week), we set the window to the number of data points collected over 168 hours, and a margin of 48 hours.
...
There are suggestions that altering the weighing can improve model convergence and decrease the uncertainty surrounding habitat preference estimates [@fieberg_how_2021]; we included this decision to examine whether the pursuit of a more confident answer is biasing the point estimate (from `r paste0(min(optionsCompleteList$area$Method_we), " to ", max(optionsCompleteList$area$Method_we))`).
...
The standard step selection model is a conditional logistic regression, where we aim to estimate the used/unused by the habitat values (case_ ~ values + strata(step_id_)).
The integrated step selection model is very similar, apart from the predictors also included step length and turn angle.
The addition of these two components is meant to reduce bias by better accounting for the selection that may simply be an artefact of the movements of the animal rather than active habitat preference [@avgar_integrated_2016].
...
Part of the motivation behind their development was to integrate autocorrelation and other critical structures of movement data into the modelling of habitat preference [@alston_mitigating_2023].
...
As a result, we have no exploration of analysis decisions for this method because aspects such an available area or points not longer apply, and availability is estimated as part of the model [@alston_mitigating_2023].
...
All models included individual and species as nested group effects (for example, the formula for the SSF model looking at absolute deviation: absDeltaEst ~ 1 + tdScaled + tfScaled + modelForm + stepDist + turnDist + availablePerStepScaled + (1|species/indi)).
We normalised all predictor variables because they all existed on vastly different scales, and we wanted to assess relative impact.
...
The analysis clostest to mirroring the agent-based model of the simulated data is the SSF.
...
Further, the simulated preference of the animal is balanced against other demands such as site fidelity, movement resistance, and avoidance of certain locations; therefore, any input habitat preference into the simulation incomparable with an derived answer from the simulated movement. 
...
The estimates calculated this way likely underestimate preference further as the rest sites were only generated within the preferred habitat (a simulated strict preference), and the no preference exploratory state that would dampen an observed preference.
The lack of direct comparability between simulated parameters and analysed estimates of preference support the use of the median estimate to explore the cause behind the most deviant estimates.
...
Based on these assessments we modified the running parameters <!-- [FINAL VALUES HERE: warmup = 200, iter = 1000, chains = 4, adapt_delta = 0.90, max_treedepth = 15] -->.

# Results

```{r targetsParse, echo=FALSE, message=FALSE}
library(dplyr)
library(stringr)
areaResults <- read.csv(here::here("data", "areaResults.csv"))
ssfResults <- read.csv(here::here("data", "ssfResults.csv"))
wrsfResults <- read.csv(here::here("data", "wrsfResults.csv"))

nDatasets <- n_distinct(areaResults$species) *
  n_distinct(areaResults$indi) *
  n_distinct(areaResults$td) *
  n_distinct(areaResults$tf)
...
The completion of the multiverse resulted in `r nDatasets` datasets representing an individual animals movements sampled via a distinct tracking regime (i.e., a combination of tracking frequency and duration).
...
`r scales::comma(nEstimates)` estimates of habitat preference (`r paste0("Wides: ", scales::comma(sum(areaResults$analysis == "wides")), "; RSF: ", scales::comma(sum(areaResults$analysis == "rsf")), "; SSF: ", scales::comma(dim(ssfResults)[1]), "; wRSF: ", scales::comma(dim(wrsfResults)[1]))`).

## General patterns

We can get an initial impression of the pattern of the final estimates of preference using a specification curve [@simonsohn_specification_2020; @hall_survey_2022].
...
```{r specCurveWides, echo=FALSE, out.width='100%', fig.height=5, fig.width=3, fig.align="centre", fig.cap="A specification curve showing all habitat preference estimates from the Wides analysis pathways. Top plot shows all estimates according to estimate, with an arbitary sorted y-position. Lower plot show the same estimates in relation to the decisions that lead to them. Left-hand numbers describe the number of NA results connected to each decision, indicating a failure somewhere in the calculations. Solid circles are the medians for each choice."}
...
```{r specCurveRSF, echo=FALSE, out.width='100%', fig.height=5, fig.width=3, fig.align="centre", fig.cap="A specification curve showing all habitat preference estimates from the RSF analysis pathways. Top plot shows all estimates according to estimate, with an arbitary sorted y-position. Lower plot show the same estimates in relation to the decisions that lead to them. Solid circles are the medians for each choice. Colours illustrate deviation from the median."}
knitr::include_graphics(here::here("notebook", "figures", "rsfSpecCurve.png"))
```

<!-- spec curve rsf -->
The Resource Selection Function (RSF) specification curve shows a more ideal *S* shape, with the majority of estimates clearly converging on a common answer (Fig. \@ref(fig:specCurveRSF)).
Similar to the Wides analysis, the majority of the answers and medians show preference for habitat 2; and while there are answers suggesting avoidance of habitat 2, all bar two medians connected to a specific choice (the lowest two tracking frequencies) suggest preference.
...
```{r uncertaintyPlot, echo=FALSE, out.width='100%', fig.height=3, fig.width=2, fig.align="centre", fig.cap="All standard errors associated with estimates of habitat preference from RSF, SSF, and wRSF analysis pathways. N.b., y axis is log scaled."}
...
There is a reasonably clear *S* shape indicating many analysis end points resulting in similar estimates of preference (Fig. \@ref(fig:specCurveSSF)).
...
```{r targetsBRMS, echo=FALSE, message=FALSE}
...
Our Bayesian models cut through the noise displayed in specification curves to reveal more generalised patterns and absorb variation stemming from the stochastic nature of the animal simulations (i.e., variation between individuals and species).
...
```{r widesModelEffects, echo=FALSE, message=FALSE}
brmWides_dEst_tdEffect <- brmEstResults %>% 
  filter(model == "wides_dEstWides") %>% 
  filter(.variable == "b_tdScaled") %>% 
  pull(inTextSummary)
brmWides_dEst_tfEffect <- brmEstResults %>% 
  filter(model == "wides_dEstWides") %>% 
  filter(.variable == "b_tfScaled") %>% 
  pull(inTextSummary)
brmWides_dEst_avpEffect <- brmEstResults %>% 
  filter(model == "wides_dEstWides") %>% 
  filter(.variable == "b_availPointsPerScaled") %>% 
  pull(inTextSummary)
brmWides_dEst_csEffect <- brmEstResults %>% 
  filter(model == "wides_dEstWides") %>% 
  filter(.variable == "b_contourScaled") %>% 
  pull(inTextSummary)

brmWides_rEst_tdEffect <- brmEstResults %>% 
  filter(model == "wides_rEstWides") %>% 
  filter(.variable == "b_tdScaled") %>% 
  pull(inTextSummary)
...
brmWides_rEst_spEffect <- brmEstResults %>% 
  filter(model == "wides_rEstWides") %>% 
  filter(.variable == "b_samplingPatternst") %>% 
  pull(inTextSummary)
```
...
For the decisions connected to Wides results, the absolute deviation was curtailed most effectively by increases in data quantity (Fig. \@ref(fig:dEstBetasWides)), with increases in tracking duration ($\beta$ `r brmWides_dEst_tdEffect`) being marginally more effective than tracking frequency ($\beta$ `r brmWides_dEst_tfEffect`).
<!-- ?????????????should we be calculating contrasts here to really support comparative statemetns like this???????????? -->
...
Contour choice increases the absolute deviation from the median estimate $\beta$ `r brmWides_dEst_csEffect`, that appears counter to the initial specification curve plot and highlights the difficulty of identifying patterns from the raw data alone. 
...
All area related choices do not move relative to each other, and reveal that MCPs as an area method to lead to lower estimates of preference in this scenario.

<!-- RSF MODELS -->

```{r rsfModelEffects, echo=FALSE, message=FALSE}
brmrsf_dEst_tdEffect <- brmEstResults %>% 
  filter(model == "rsf_dEstRSF") %>% 
  filter(.variable == "b_tdScaled") %>% 
  pull(inTextSummary)
...
brmrsf_dEst_akdeEffect <- brmEstResults %>% 
  filter(model == "rsf_dEstRSF") %>% 
  filter(.variable == "b_areaAKDE") %>% 
  pull(inTextSummary)
brmrsf_dEst_kdeEffect <- brmEstResults %>% 
  filter(model == "rsf_dEstRSF") %>% 
  filter(.variable == "b_areaKDEhref") %>% 
  pull(inTextSummary)
brmrsf_dEst_dbmEffect <- brmEstResults %>% 
  filter(model == "rsf_dEstRSF") %>% 
  filter(.variable == "b_areadBBMM") %>% 
  pull(inTextSummary)

brmrsf_rEst_tdEffect <- brmEstResults %>% 
  filter(model == "rsf_rEstRSF") %>% 
  filter(.variable == "b_tdScaled") %>% 
  pull(inTextSummary)
...
The two largest effects are from tracking duration ($\beta$ `r brmrsf_dEst_tdEffect`) and frequency ($\beta$ `r brmrsf_dEst_tfEffect`), showing that more data leads to more stable estimates of habitat preference (i.e., more likely to get an answer closer to the overall median estimate).
...
<!-- model res rsf raw -->
The RSF model examining raw deviation from the median estimate reveals a general tendency for higher data quantity to lead to more conservative estimates of preference (tracking duration $\beta$ `r brmrsf_rEst_tdEffect` and tracking frequency $\beta$ `r brmrsf_rEst_tfEffect`; Fig. \@ref(fig:rEstBetasRSF)). 
While the simulation parametrisation makes it impossible to definitively determine whether this conservatism is correct, the direct estimates from the simulations using an RSF approach suggest the low positive estimates (also near the median <!--[NEED LINK TO SUPP MAT HERE]-->) are more likely to be the correct estimation of simulated preference via RSF.
This matches the absolute deviation model where more data leads to less deviant estimations.

Stratified available point sampling was associated with more positive estimates of preference ($\beta$ `r brmrsf_rEst_spEffect`), but also those closer to the median (as indicated by the previous model).
...
Dynamic Brownian Bridge Movement models were connected to the more variable estimates, but this model reveals that the variability did not tend towards a particular direction ($\beta$ `r brmrsf_rEst_dbmEffect`).
...
Taken together it appears that methods capturing a spatially broader picture of availability have a tendency to produce more stable (in relation to other analysis decisions) but higher estimates of preference.
...
```{r ssfModelEffects, echo=FALSE, message=FALSE}
brmssf_dEst_tdEffect <- brmEstResults %>% 
  filter(model == "ssf_dEstSSF") %>% 
  filter(.variable == "b_tdScaled") %>% 
  pull(inTextSummary)
...
brmssf_rEst_sdEffect <- brmEstResults %>% 
  filter(model == "ssf_rEstSSF") %>% 
  filter(.variable == "b_stepDistgamma") %>% 
  pull(inTextSummary)
```
...
The SSF absolute deviation model very closely matches the previous two models (Fig. \@ref(fig:dEstBetasSSF)), where tracking quantity both in terms of duration ($\beta$ `r brmssf_dEst_tdEffect`) and frequency ($\beta$ `r brmssf_dEst_tfEffect`) dominate in regards to the explaining why estimates tend to be closer to the median estimate.
...
The only noticeable difference is in the step distribution decision that changes the direction of the beta point estimate from negative to positive ($\beta$ `r brmssf_rEst_sdEffect`).
...
```{r wrsfModelEffects, echo=FALSE, message=FALSE}
brmwrsf_dEst_tdEffect <- brmEstResults %>% 
  filter(model == "wrsf_dEstwrsf") %>% 
  filter(.variable == "b_tdScaled") %>% 
  pull(inTextSummary)
...
Both tracking duration and tracking frequency lead to lower deviation from the median estimate (tracking duration $\beta$ `r brmwrsf_dEst_tdEffect` and tracking frequency $\beta$ `r brmwrsf_dEst_tfEffect`), and the nearly exclusively positively skewed outliers mean that lower raw deviation is associated with lower estimates (tracking duration $\beta$ `r brmwrsf_rEst_tdEffect` and tracking frequency $\beta$ `r brmwrsf_rEst_tfEffect`; Fig. \@ref(fig:dEstBetasWRSF)).
...
In particular the benefits of larger datasets and the importance of defining availability [@street_solving_2021; @northrup_practical_2013], the latter particularly key because of the potential for subjective definitions [@beyer_interpretation_2010]. 
...
The limited cost of integrating step and turn into SSF formula would suggest iSSF formulation is likely always preferable for estimation of individual habitat preference.
...
Improvement will only come with the additions being fundamentally more accurate, and better reflective the study system [@devezer_case_2021].
...
Scale remains a key consideration for any study [@thurfjell_applications_2014; @northrup_conceptual_2022], and context-specific selection remains difficult to translate [@avgar_habitat_2020].
...
More formal and mathematically based estimates of reliability are available for specific approaches, for example RSFs [@street_solving_2021], and these present an important component when reporting RSFs. 
...
There are already methods for assessing require sample size to detected a given selection for RSFs [@street_solving_2021].
...
```{r r2table, echo=FALSE, message=FALSE}
knitr::kable(
  brmsR2Results %>% 
    mutate(across(where(is.numeric), round, digits = 3)) %>% 
    mutate(model = sub("_r2$", "", model),
           response = ifelse(str_detect(model, "dEst"), "Absolute Deviation", "Raw Deviation"),
           method = sub("_", "", str_extract(model, "^.*_")),
           Component = str_to_sentence(Component)
           ) %>% 
    select(-model),
  caption = "$All R^2 values for Bayesian regression models.$", align = "c",
