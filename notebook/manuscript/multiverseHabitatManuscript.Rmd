...
brmWides_dEst_tdEffect <- brmEstResults %>% 
  dplyr::filter(model == "wides_dEstWides") %>% 
...
brmWides_rEst_tdEffect <- brmEstResults %>% 
  dplyr::filter(model == "wides_rEstWides") %>% 
  dplyr::filter(.variable == "b_tdScaled") %>% 
  pull(inTextSummary)
...
brmrsf_dEst_tdEffect <- brmEstResults %>% 
  dplyr::filter(model == "rsf_dEstRSF") %>% 
  dplyr::filter(.variable == "b_tdScaled") %>% 
  pull(inTextSummary)
...
brmrsf_rEst_tdEffect <- brmEstResults %>% 
  dplyr::filter(model == "rsf_rEstRSF") %>% 
  dplyr::filter(.variable == "b_tdScaled") %>% 
  pull(inTextSummary)
...
brmssf_dEst_tdEffect <- brmEstResults %>% 
  dplyr::filter(model == "ssf_dEstSSF") %>% 
  dplyr::filter(.variable == "b_tdScaled") %>% 
  pull(inTextSummary)
...
brmssf_rEst_sdEffect <- brmEstResults %>% 
  dplyr::filter(model == "ssf_rEstSSF") %>% 
  dplyr::filter(.variable == "b_stepDistgamma") %>% 
  pull(inTextSummary)
```

<!-- model res ssf absolute -->

The SSF absolute deviation model very closely matches the previous two models (Fig. \@ref(fig:brmsAllEffectsPlot); Fig. \@ref(fig:dEstBetasSSF)), where tracking quantity both in terms of duration ($\beta$ `r brmssf_dEst_tdEffect`) and frequency ($\beta$ `r brmssf_dEst_tfEffect`) dominate in regards to the explaining why estimates tend to be closer to the median estimate.
...
brmwrsf_dEst_tdEffect <- brmEstResults %>% 
  dplyr::filter(model == "wrsf_dEstwrsf") %>% 
  dplyr::filter(.variable == "b_tdScaled") %>% 
  pull(inTextSummary)
...
brmwrsf_rEst_tdEffect <- brmEstResults %>% 
  dplyr::filter(model == "wrsf_rEstwrsf") %>% 
  dplyr::filter(.variable == "b_tdScaled") %>% 
  pull(inTextSummary)
...
knitr::kable(
  brmsR2Results %>% 
    ...
    dplyr::select(-model),
  caption = "$All R^2 values for Bayesian regression models.$",
  format = "latex",
  align = "lcc",
  vline = "",
  toprule = "",
