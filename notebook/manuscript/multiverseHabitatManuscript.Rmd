...
<!-- POSSIBLE EXPANSION - The current multiverse(s) are examining movement data with a known positive selection, but it would be very easy to re-run the whole thing with no selection. I think this could be done by just scrambling the classified raster used in the habitat selection analyses –breaking any connection between the movement (and the underlying foraging, resting, movement layers) and the simplified layers used for the analyses. I am going to focus on getting what I have tidied up and the preprint better before exploring this, but it does strike me as an interesting and simple angle to explore: what does the variation look like when there is no selection? Adding this to the paper I think would give more of an insight into whether the variation can cause false positives; as it stands now, the papers are more about spotting the variation causing false negatives (given a certain hard to define level of habitat selection). So in lieu of being able to run a suite of scenarios with different habitat selection strengths, a comparison of zero selection to some-positive selection might be the next best thing. -->
...
For each method, we ran two Bayesian regression models to explore: 1) which decisions best explain the absolute deviation from the median estimated selection, 2) which decisions best explain the transformed deviation from the median estimated selection.
The first model provides us with an idea of the decisions that can lead to random, but potentially unbiased noise surrounding the median estimate; whereas the second model highlights decisions that lead to systematic over or under estimation of a habitat selection.
...
We elected to have the model try and predict the deviation from the median estimate of habitat selection because the simulation cannot provide a direct analogous value to the outputs of all three methods.
...
The estimates calculated this way likely underestimate selection further as the rest sites were only generated within the preferred habitat (a simulated strict selection), and the no selection exploratory state that would dampen an observed selection.
...
We can get an initial impression of the pattern of the final estimates of selection using a specification curve [@simonsohn_specification_2020; @hall_survey_2022].
...
Similar to the Wides analysis, the majority of the answers and medians show selection for habitat 2; and while there are answers suggesting avoidance of habitat 2, all bar two medians connected to a specific choice (the lowest two tracking frequencies) suggest selection.
...
```{r specCurveSSF, echo=FALSE, out.width='100%', fig.height=5, fig.width=3, fig.align="centre", fig.cap="A specification curve showing all habitat selection estimates from the SSF analysis pathways. Top plot shows all estimates according to estimate, with an arbitary sorted y-position. Lower plot show the same estimates in relation to the decisions that lead to them. Solid circles are the medians for each choice. Colours illustrate deviation from the median."}
...
Every decision examined by the RSF absolute effect model appears to be make a difference when estimating selection (Fig. \@ref(fig:dEstBetasRSF)).
...
<!-- model res rsf raw -->
The RSF model examining raw deviation from the median estimate reveals a general tendency for higher data quantity to lead to more conservative estimates of selection (tracking duration $\beta$ `r brmrsf_rEst_tdEffect` and tracking frequency $\beta$ `r brmrsf_rEst_tfEffect`; Fig. \@ref(fig:rEstBetasRSF)). 
...
Stratified available point sampling was associated with more positive estimates of selection ($\beta$ `r brmrsf_rEst_spEffect`), but also those closer to the median (as indicated by the previous model).
...
Despite the inherit limitations of multiverses, the exploration of analysis choice presents an important consideration for any researcher looking to know the reliability of their findings [@hoffmann_multiplicity_2021], in particular model, method, data preprocessing uncertainty (Table \@ref(tab:sourcesVariation)).
