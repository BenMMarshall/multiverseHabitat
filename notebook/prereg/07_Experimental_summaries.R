...
(splitSpecCurve <- rsfResultsPlotData %>%
    ggplot() +
...
(overallSpecCurve <- rsfResults %>%
    ...
    geom_point(aes(x = Estimate, y = index, colour = sigColour),
               size = 1, alpha = 0.2)+
    geom_point(data = data.frame("medEst" = median(rsfResults$Estimate),
...
(splitSpecCurve_wides <- widesResultsPlotData %>%
    ...
    geom_hline(yintercept = seq(0.5,10.5,1), linewidth = 0.5, alpha = 0.25, colour = "#403F41",
               linetype = 2) +
    geom_text(data = medData, aes(x = -0.25, y = value, label = nZeroNA),
              colour = palette["coreGrey"], fontface = 4) +
    facet_wrap(.~variable, ncol = 1, scales = "free_y", strip.position = "left") +
    labs(y = "", x = "Estimate") +
    # scale_colour_manual(values = unname(palette[c("2", "coreGrey", "BADGER")]), na.value = "#000000") +
    theme_bw() +
...
(overallSpecCurve_wides <- widesResults %>%
    ...
    geom_point(data = data.frame("medEst" = median(widesResults$Estimate),
    ...
    geom_point(data = data.frame("medEst" = median(widesResults$Estimate),
               ...
               aes(x = medEst, y = indexLoc),
               alpha = 1, size = 2, colour = "#403F41") +
    geom_text(data = medData, aes(x = -0.25, y = 1, label = sum(nZeroNA)),
              colour = palette["coreGrey"], fontface = 4) +
    # scale_colour_manual(values = unname(palette[c("2", "coreGrey", "BADGER")]), na.value = "#000000") +
    labs(y = "", x = "Estimate") +
    theme_bw() +
...
overallSpecCurve_wides / splitSpecCurve_wides +
  plot_layout(heights = c(1, 3), guides = "collect")
