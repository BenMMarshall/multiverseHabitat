...
Specifically, we will explore the decisions made during the workflows using adehabaitatHS [@adehabitatHS], amt [@amt], and ctmcmove [@ctmcmove] R packages.
...
To account for the variation potentially introduced from smoothing factor choice, we will included two bandwidth selection processes commonly used in home range analyses [@crane_lots_2020] <!--more refs for other groups?-->: the reference bandwidth (href), and Least Squares Cross Validation (LSCV).

The limitations of MCPs and KDEs have prompted the development of newer methods of area use estimation [@silva_reptiles_2020], which better account for the non-independence and autocorrelative structures within animal movement data [@Noonan2018].
